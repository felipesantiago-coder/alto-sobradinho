...
// Dados corretos da tabela XLS fornecida na conversa
const tabelaValores = {
  // Bloco A - Térreo
...
// Encontrar e substituir os valores de venda do Bloco A
Object.keys(tabelaValores).forEach(unitKey => {
  if (content.includes(`'${unitKey}':`)) {
    const regex = new RegExp(`'${unitKey}': {[^}]*valor: '[^}]*'[^}]*'`);
    const match = content.match(regex);
    if (match) {
      const newValor = tabelaValores[unitKey];
