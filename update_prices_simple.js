...
// Valores corretos da tabela XLS - usando dados reais do arquivo fornecido
const tabelaValores = {
  // Bloco A - Térreo
...
// Função para substituir valores de forma mais simples
function substituirValores(content, bloco, valores) {
  console.log(\`Atualizando valores do Bloco \${bloco}...`);
  Object.keys(valores).forEach(unitKey => {
    if (content.includes(\`'${bloco}-${unitKey}: \`)) {
      const oldValor = valores[unitKey];
...
// Atualizar valores dos Blocos B, C e D
...
console.log('Resumo:');
console.log('- Bloco A: ' + Object.keys(tabelaValores.A).length + ' unidades');
console.log('- Bloco B: ' + Object.keys(valoresAdicionais.B).length + ' unidades');
console.log('- Bloco C: ' + Object.keys(valoresAdicionais.C).length + ' unidades');
console.log('- Bloco D: ' + Object.keys(valoresAdicionais.D).length + ' unidades');
"